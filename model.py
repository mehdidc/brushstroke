...
class BrushStroke(nn.Module):
    def __init__(
        self, image_w, image_h, pad="half_patch", eps=1e-7, sigma=0.2, device="cpu"
    ):
        super().__init__()
    ...
    def forward(self, brushes, patches):
        ...
        ux = gx.view(gx.size(0), gx.size(1), 1) + ((widths - (pw / 2) - 0.5) * sx).view(
            1, 1, -1
        )
        ...
        Fx = Fx.view(Fx.size(0), Fx.size(1), 1, Fx.size(2), 1, Fx.size(3))
        out = (Fx * p).sum(dim=-1)
        out = out.view(
            out.size(0), out.size(1), out.size(2), out.size(3), out.size(4), 1
        )
        Fy = Fy.transpose(2, 3)
        Fy = Fy.view(Fy.size(0), Fy.size(1), 1, 1, Fy.size(2), Fy.size(3))
...
class BrushAE(nn.Module):
    def __init__(
        ...
        nf = ndf
        layers = [nn.Conv2d(nb_colors, nf, 4, 2, 1, bias=True), nn.ReLU(True)]
        for i in range(nb_layers - 1):
            layers.extend([nn.Conv2d(nf, nf * 2, 4, 2, 1, bias=True), nn.ReLU(True)])
            if i < nb_layers - 1:
                nf *= 2
        ...
        self.patch_predictor_layer1 = nn.Sequential(
            nn.Linear(hsize, 512),
            nn.ReLU(True),
            nn.Linear(512, nb_patches * patch_embedding_size),
        )
        self.patch_predictor_layer2 = nn.Sequential(
            nn.Linear(patch_embedding_size, 512),
            nn.ReLU(True),
            nn.Linear(512, nb_patches * patch_embedding_size),
        )
        ...
        self.pos_predictor_layer1 = nn.Linear(hsize, nb_patches * 5)
        self.pos_predictor_layer2 = nn.Linear(patch_embedding_size, nb_patches * 5)
    ...
    def encode(self, x):
        ...
        patches = self.patch_predictor_layer1(h)

        patches = patches.view(B, self.nb_patches, self.patch_embedding_size)
        patches = patches.view(B * self.nb_patches, -1)
        patches_layer2 = patches


        patches = self.patch_predictor_layer2(patches)
        patches = patches.view(B * self.nb_patches * self.nb_patches, -1)
        patches = self.patch_embedding(patches)
        patches = nn.Sigmoid()(patches)
        patches = patches.view(
            B, 
            self.nb_patches * self.nb_patches, 
            C, 
            self.patch_size, 
            self.patch_size
        )
        pos1 = self.pos_predictor_layer1(h)
        pos1 = pos1.view(B, self.nb_patches, 1, 5)
...
if __name__ == "__main__":
    ...
    ae = BrushAE(nb_patches=6, nb_colors=1, patch_size=4, image_size=32)
    y = ae(x).detach().numpy()
    y /= y.max()
    y = 1 - y
    im = y[0, 0]
